...
function extractMedicineDetails(text: string): MedicineDetails[] {
  ...
  // New format parser for the updated system prompt output
  // Format: [Medicine Name] ([Dosage]): [Frequency], [Timing], [Special Instructions], for [Duration].
  const newFormatRegex = /([^(]+)\s*\(([^)]+)\):\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*for\s*([^.]+)/gi;
  ...
  while ((match = newFormatRegex.exec(text)) !== null) {
    ...
    const timing = match[4].trim();
    const specialInstructions = match[5].trim();
    ...
    if (durationText.toLowerCase().includes('month')) {
      duration = 30;
    } else if (durationText.match(/(\d+)\s*days?/i)) {
      duration = parseInt(durationText.match(/(\d+)\s*days?/i)![1]);
    } else if (durationText.match(/(\d+)\s*weeks?/i)) {
      duration = parseInt(durationText.match(/(\d+)\s*weeks?/i)![1]) * 7;
    }
    
    // Format dosage instruction
    let dosageInstruction = `${frequency}, ${timing}`;
    if (specialInstructions) {
      dosageInstruction += `, ${specialInstructions}`;
    }
    
    medicines.push({
      name: `${medicineName} (${dosage})`,
      dosage: dosageInstruction,
      frequency: determineFrequency(frequency, timing),
      duration: duration,
      notes: `${specialInstructions}, for ${durationText}`
    });
  }
  
  // If no matches with new format, try previous formats
  if (!foundMatches) {
    // Try Format 1: The format with "* **TAB. MEDICINE_NAME**" and indented dosage/duration with +
    ...
    // If still no medicines found, try Format 2
    ...
    // If still no matches found, try to parse table or other formats
    if (medicines.length === 0) {
...
// Helper function to determine frequency from text
function determineFrequency(frequency: string, timing: string): string {
  const frequencyLower = frequency.toLowerCase();
  const timingLower = timing.toLowerCase();
  
  if (frequencyLower.includes('twice daily') || frequencyLower.includes('two times')) {
    return "twice daily";
  } else if (frequencyLower.includes('thrice daily') || frequencyLower.includes('three times')) {
    return "thrice daily";
  } else if (frequencyLower.includes('every') && frequencyLower.includes('hour')) {
    const hourMatch = frequencyLower.match(/every\s+(\d+)\s*hours?/i);
    return hourMatch ? `every ${hourMatch[1]} hours` : "once daily";
  } else {
